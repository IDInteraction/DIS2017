...
logConn <- file("Runlog.txt")
file.copy("RNGstate.dontchange", "RNGstate")
writeLines("Copied RNGstate.dontchange to RNGstate for run", con = logConn)
@
...
for (i in 1:nrow(configurations)) {
  ...
  write(paste(configurations[i,], collapse = ":"), logConn, append = TRUE)
  runoutput <- tempfile()
...
accuracyfigure <- ggplot(data = resultsmean, aes(x = frames, y = avggt, colour = config)) + geom_line() + geom_line(aes(y = avgxval), linetype = 2) +   facet_wrap( experimentpart ~ tracker)
