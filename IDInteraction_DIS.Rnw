...
We also plan to investigate using the remaining sensor data (e.g.\ accelerometer and Kinect data) to improve the accuracy of our predictions.  We note that the ``tracking'' and ``classification'' parts of our approach are distinct, and that other, domain specifc ``tracking'' approaches may also be well suited to generate data for the classifer.
