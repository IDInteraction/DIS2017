...
We also plan to investigate using the remaining sensor data (e.g.\ the sensor and Kinect data) to improve the accuracy of our predictions.  We note that the ``tracking'' and ``classification'' parts of our approach are distinct, and that other, domain specified, ``tracking'' approaches may also be well suited to generate data for the classifier.
