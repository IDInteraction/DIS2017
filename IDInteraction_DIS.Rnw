...
For \Sexpr{numparticipants} of the participants\footnote{Work to encode the remaining participants is ongoing.}, the web-cam video was manually encoded using \texttt{ELAN} \cite{ELAN} %,  software to annotate video and audio resources, 
