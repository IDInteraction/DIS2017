...
When the classifier is operating in non-interactive mode, we know the ground-truth state for each frame, which we can use to evaluate the classifier's performance.  When using the classifier ``for real'', in interactive mode, such data will not, in general be available. When training from a random sample of frames, we can exploit the randomness of the sample to estimate the classifier accuracy usings a shuffle split cross validation approach.  This takes a randomly selected 50\% of the frames as holdback, fits the classifer on the remaining frames, and calculates the accuracy. We report the mean of 1000 such evaluations. We cannot use this approach when classifying frames sequentially, since our training data are not a random sample of the video's frames; such an approach would inflate the estimated accuacy. When running in interactive mode the user can also visually assess the classifier's performance by replaying the video, with a rectangle, whose colour indicates the predicted behaviour overlaid on each frame \textit{Include a figure??}.  If necessary they can then classify further frames, until the required accuracy is reached. 
...
% TODO sort out random seed; need to add seed to github, and then
% load at start of each iter and save at end
...
for (i in 1:nrow(configurations)) {
  ...
  resultsframe <- rbind(resultsframe, 
                        data.frame(c(configurations[i,], 
                          crossvalAccuracy = results[1,"crossvalAccuracy"],
                          groundtruthAccuracy = results[1, "groundtruthAccuracy"])))
  unlink(runoutput)
}
...
resultsmean <- resultsframe %>% group_by(config, frames, experimentpart) %>% summarise(avgxval = mean(crossvalAccuracy),
                                                        avggt = mean(groundtruthAccuracy))

accuracyfigure <- ggplot(data = resultsmean, aes(x = frames, y = avggt, colour = config)) + geom_line() + geom_line(aes(y = avgxval), linetype = 2) +   facet_wrap(~ experimentpart)

@

<<resultsfig, echo=FALSE, width='1.3\\columnwidth', fig.cap="Test caption">>=
